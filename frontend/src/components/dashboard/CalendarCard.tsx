...
type CalEvent = { id: string; date: string /* YYYY-MM-DD */; title: string; category?: string; notes?: string }
...
const CalendarCard = ({
  ...
  const days = useMemo(() => getMonthDays(displayedYear, displayedMonth), [displayedMonth, displayedYear])
      ...
      <div className="mt-6">
        ...
        <div className="mt-3 space-y-3 text-sm text-slate-600">
          {(eventsByDate.get(formatDateKey(selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate())) ?? []).length === 0 ? (
            ...
            (eventsByDate.get(formatDateKey(selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate())) ?? []).map((ev) => (
